...
function sendLeaveword() {
    ...
    var stxtBeizhu = $("#txtBeizhu").val(); //留言信息

    $("#ErrorInfo").removeAttr("class").addClass("red pl10");
    var zz = /^1[3|4|5|8][0-9]\d{4,8}$/;
    if (sUserName.length == 0) {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的姓名!");
        return;
    }
    else if (sTel.length == 0) {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的联系电话号码!");
        return;
    }
    else if (sWxh.length == 0) {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的QQ/微信号码!");
        return;
    }
    else if (!zz.test(sTel)) {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("联系电话格式不正确!");
        return;
    }
    else if (stxtBeizhu.length == 0) {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的留言信息!");
        return;
    }
    else {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("正在提交数据...");
...
function sendLeaveword1() {
    ...
    $("#ErrorInfo").removeAttr("class").addClass("red pl10");
    var zz = /^1[3|4|5|8][0-9]\d{4,8}$/;
    if (sUserName.length == 0 || sUserName.length == "请输入您的姓名") {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的姓名!");
        return;
    }
    else if (sTel.length == 0 || sTel.length == "请输入您的电话") {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的电话!");
        return;
    }
    else if (sWxh.length == 0 || sWxh.length == "请输入您的QQ/微信号") {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的QQ/微信号!");
        return;
    }
    else if (!zz.test(sTel)) {
        $("#ErrorInfo").empty();
    ...
    else if (stxtBeizhu.length == 0 || stxtBeizhu.length == "请输入您的留言内容") {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入您的留言内容!");
        return;
    }
    else {
        $("#ErrorInfo").empty();
        ...
        $.post("/Mobile/MAjax.ashx?action=SendLeaveword&t=" + Math.random(), {
            ...
            if (sta == "1") {
                ...
                $("#ErrorInfo").show(1000, function () {
                    ...
                    $("#UserTel").val("请输入您的电话");
                    $("#UserWxh").val("请输入您的QQ/微信号");
                    $("#txtBeizhu").val("请输入您的留言内容");
                });
            } else {
